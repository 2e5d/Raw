--[[
    GEMINI MULTI-FUNCTIONAL ESP
    Integrated with Global Settings for UI Menu Support
]]
...
-- 2. GLOBAL SETTINGS (Link your UI Menu to these)
_G.ESPLibrary = {
    Settings = {
        Enabled = true,
        FPSMode = false,        -- Disables complex rendering for performance
        MaxDistance = 2500,     -- Render distance in studs
        ...
        -- BOX & STYLE
        BoxThickness = 1.8,
        CornerRadius = 12,      -- Roundness of the box
        Quality = 8,            -- Segment count for curves
        Rounded = true,
        FillEnabled = true,     -- Internal scanlines
        FillDensity = 35,
...
local function UpdateESP(obj, pos, size, topColor, healthPercent, char, playerName)
    ...
    if s.ShowName then
        ...
        obj.Name.Font = s.NameBold and 3 or 2
    ...
    if s.ShowTracer then
        ...
        local target = pos + (size / 2)
        local fullVec = target - origin
        for i, seg in ipairs(obj.TracerSegments) do
            ...
            seg.Color, seg.Transparency, seg.Visible = botColor:Lerp(topColor, tS), smoothPulse, true
        end
    else
    ...
    if s.FillEnabled and not s.FPSMode then
        for i, line in ipairs(obj.FillLines) do
            ...
            line.Color, line.Transparency, line.Visible = topColor:Lerp(botColor, t), smoothPulse * 0.5, true
        end
    else
        for _, v in pairs(obj.FillLines) do v.Visible = false end
    end

    -- ROUNDED BORDER RENDERING
    for i = 1, q do
        ...
        for _, p in ipairs(group) do 
            p.Transparency, p.Thickness, p.Visible = borderPulse, s.BoxThickness, true
            p.Color = (p == obj.L[i] or p == obj.R[i]) and topColor:Lerp(botColor, t) or ( (p == obj.TL[i] or p == obj.TR[i]) and topColor or botColor )
        end
    end
        ...
        if char and char:FindFirstChild("HumanoidRootPart") and char:FindFirstChild("Humanoid") then
            ...
            if onScreen then
                ...
                if dist < s.MaxDistance then
                    ...
                    local bPos = Vector2.new(screenPos.X - w/2, screenPos.Y - h/2)
...
local function CleanupPlayer(p)
    if ESPTable[p] then
        ...
        for _, drawing in ipairs(data.AllDrawings) do drawing:Remove() end
        ESPTable[p] = nil
    end
end
