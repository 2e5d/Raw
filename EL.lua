...
-- Global GUI Container
local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "GranitESP_Fills"
ScreenGui.DisplayOrder = -1 -- Keeps fill behind the drawing lines
...
local function CreateESP(player)
    ...
    obj.FillCorner.CornerRadius = UDim.new(0, _G.ESPLibrary.Settings.CornerRadius)
    obj.FillCorner.Parent = obj.FillFrame
...
local function UpdateESP(obj, pos, size, topColor, healthPercent, char, playerName)
    ...
    if s.FillEnabled then
        ...
        obj.FillCorner.CornerRadius = UDim.new(0, r)
    ...
    -- Drawing Logic (Name, Skeleton, Tracer, Rounded Box)
...
local Connection = RunService.RenderStepped:Connect(function()
    ...
    for player, obj in pairs(ESPTable) do
        ...
        if s.Enabled and char and char:FindFirstChild("HumanoidRootPart") and char:FindFirstChild("Humanoid") then
            ...
            if onScreen and (Camera.CFrame.Position - hrp.Position).Magnitude < s.MaxDistance then
                ...
                UpdateESP(obj, Vector2.new(pos.X - (3.5 * scale)/2, pos.Y - (5.5 * scale)/2), Vector2.new(3.5 * scale, 5.5 * scale), player.TeamColor.Color, hum.Health/hum.MaxHealth, char, player.Name)
            else
                for _, d in pairs(obj.AllDrawings) do d.Visible = false end
                obj.FillFrame.Visible = false
